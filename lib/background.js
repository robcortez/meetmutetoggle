...
function checkRemovedTab(tabid, removeInfo) {
  chrome.storage.local.get('meetings', function (result) {
    ...
    if (tabIndex !== -1) { // tab is a meet
      // tab closed or navigated away from
...
function monitorMeeting(meeting) {
  chrome.storage.local.get('meetings', function(result) {
    ...
    chrome.storage.local.set({meetings: meetings}, function() {
      chrome.storage.local.get('meetings', function (r) {
        console.log("Meetings updated:", r.meetings)
      })
    })
  })
}
...
chrome.runtime.onMessage.addListener(
  function(request, sender, sendResponse) {
    if (request.action === 'leaving') {
      console.log(`Navigating away from ${sender.tab.id}`)
...
// redundant due to beforeunload listener in content script
chrome.tabs.onRemoved.addListener(checkRemovedTab)